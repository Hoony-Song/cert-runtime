...
show_help() {
  cat <<'USAGE'
사용법: create-vm-cluster.sh --session-id <id> [--exam-type <type>] [--exam-set-id <id>] [--session-disk-dir <dir>] [--session-root <dir>] [--cp-disk <path>] [--worker-disk <path>] [--cp-cloud-init-iso <path>] [--worker-cloud-init-iso <path>] [--network <name>] [--cp-vcpus <n>] [--worker-vcpus <n>] [--cp-memory-mib <mib>] [--worker-memory-mib <mib>] [--os-variant <name>] [--dry-run] [--verbose]

세션별 kubeadm control-plane VM과 worker VM을 libvirt로 생성하고 부팅 상태를 확인한다.
USAGE
}
...
CP_MEMORY_MIB="${CKA_VM_CP_MEMORY_MIB:-4096}"
WORKER_MEMORY_MIB="${CKA_VM_WORKER_MEMORY_MIB:-4096}"
...
CP_VM_NAME="cka-${SESSION_ID}-cp"
WORKER_VM_NAME="cka-${SESSION_ID}-worker"
...
if [[ -z "${CP_CLOUD_INIT_ISO}" ]]; then
  CP_CLOUD_INIT_ISO="${SESSION_ROOT}/${SESSION_ID}/cloud-init/cp/${SESSION_ID}-cp-cloud-init.iso"
fi

if [[ -z "${WORKER_CLOUD_INIT_ISO}" ]]; then
  WORKER_CLOUD_INIT_ISO="${SESSION_ROOT}/${SESSION_ID}/cloud-init/worker/${SESSION_ID}-worker-cloud-init.iso"
fi
...
if [[ "${DRY_RUN}" == true ]]; then
  printf '{"sessionId":"%s","examType":"%s","examSetId":"%s","vms":[{"name":"%s","disk":"%s","cloudInitIso":"%s","vcpus":%s,"memoryMiB":%s},{"name":"%s","disk":"%s","cloudInitIso":"%s","vcpus":%s,"memoryMiB":%s}],"network":"%s","dryRun":true}\n' \
    "${SESSION_ID}" "${EXAM_TYPE}" "${EXAM_SET_ID}" \
    "${CP_VM_NAME}" "${CP_DISK}" "${CP_CLOUD_INIT_ISO}" "${CP_VCPUS}" "${CP_MEMORY_MIB}" \
    "${WORKER_VM_NAME}" "${WORKER_DISK}" "${WORKER_CLOUD_INIT_ISO}" "${WORKER_VCPUS}" "${WORKER_MEMORY_MIB}" \
    "${NETWORK}"
  exit 0
fi
...
if vm_exists "${CP_VM_NAME}" || vm_exists "${WORKER_VM_NAME}"; then
...
if [[ "${VERBOSE}" == true ]]; then
  printf '{"sessionId":"%s","controlPlaneVm":"%s","workerVm":"%s","network":"%s","created":true}\n' \
    "${SESSION_ID}" "${CP_VM_NAME}" "${WORKER_VM_NAME}" "${NETWORK}"
fi
